...
export default class ModalBusinessProcess extends LightningElement {
        ...
        if (result.data) {
            ...
            for(var key in conts){
                ...
                if(key == 'Account') {
                    ...
                    this.mapRelatedObjectsData.push({value:manageOther, key:key}); //Here we are creating the array to show on UI.
                }
                
                
            }
            this.mapRelatedObjectsData.push({value:'Manage Task', key:'Task'});
        }
    }
    ...
    @wire(getRecordData,{recordId: '$recordId',  cols: 'AccountId'}) wiredAccounts ({ error, data }) {
        if (data) {
            console.log('ggggg1');
            console.log(JSON.stringify(data[0].AccountId));
