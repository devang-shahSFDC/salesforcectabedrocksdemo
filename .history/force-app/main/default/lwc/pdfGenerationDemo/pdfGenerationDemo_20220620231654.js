...
import addImageToPDF from '@salesforce/apex/AddImageToTheExistingPDF.addImageToPDF'
export default class PdfGenerationDemo extends LightningElement {
    ...
    pdfHandler(){
        ...
        generatePDF({ recordId:this.recordId, htmlData:content.outerHTML}).then(result=>{
            ...
            addImageToPDF({SourceFileUrl:'https://democom60-dev-ed.lightning.force.com/servlet/servlet.FileDownload?file=${result.Id}'})
            window.open(`https://democom60-dev-ed.lightning.force.com/servlet/servlet.FileDownload?file=${result.Id}`)
        }).catch(error=>{
            console.error(error)
        })
    }
